...
|  6   | Use Odometry           | マッチングにオドメトリデータの利用有無を設定。（注： /vehicle/odomトピックを参照している。） |  -   | チェックあり |
|  7   | Use IMU                | マッチングにIMUセンサデータの利用有無を設定。                |  -   | チェックあり |
...
※本項目にチェックを入れると、TFが変化する**（Autowere側で［map］→［base_link］のTFを生成する）**。そのため、元々あった［odom］→［base_link］間のTFと重なり、mapマッチングがちらつくことになる。スタティックのTF設定では、［odom］［base_footprint］は定義しないようにする。（実際には/vehcle/odom）
