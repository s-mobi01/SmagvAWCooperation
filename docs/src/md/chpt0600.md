...
|   4   | ↑                                               |     ↑     |   0.965   | base_footprintなし（base_linkが地面）         |
...
**（smagvの場合では現状、起動launchで発行しているので以下操作はたぶん不要。Logieeの場合は発行していないので適宜設定する。）**
...
|  9   | Use Odometry           | MAP作成にオドメトリデータの利用有無を設定。（注： /vehicle/odomトピックを参照している。） |  -   | チェックあり |
|  10  | Use IMU                | MAP作成にIMUセンサデータの利用有無を設定。                   |  -   | チェックあり |
|  11  | IMU topic              | IMUデータのトピック名を選択する。（クォータニオン（orientation）データのセットされているトピック。/imu/rawデータではたぶんダメ） |  -   | /imu/data    |
