...
AGVにて障害物として検知しない距離の設定を行う。（無視する半径）
...
| 格納先     | ~/Autoware/ros/install/waypoint_planner/share/waypoint_planner/launch |
...
<launch>
  ...
  <arg name="remove_points_upto" default="0.5" /> ←ここの値を変更する
...
|  3   | Points Topic                        | 物体検出に利用する点群トピック名を指定する                   |   -   |        |
|  4   | Enable Planner dynamical switch     | waypointを動的に書き換えるかどうかを指定する                 |   -   | 1.10.0 |
|  5   | Stop Distance for Obstacle          | 経路上の物体を検知した際の前方障害物までの停止距離を指定する |   m   |        |
|  6   | Stop Distance for Stopline          | 経路上の停止線までの停止距離を指定する                       |   m   |        |
|  7   | Detection Range                     | 経路上の物体を検出して停止する際の検出範囲を指定する         |   m   |        |
|  8   | Deceleration Range                  | 経路上の物体を検出して減速する際の検出範囲を指定する         |   m   |        |
|  9   | Points Threshold                    | 経路上の物体うぃ検出する際の点の数のしきい値を指定する。この値以上の点が検出範囲内にあれば物体を検出する。 |  個   |        |
|  10  | Detection Height Top                | 検出する点群の高さの上限を指定する。高さは点群の検出に用いているセンサからの距離。 |   m   |        |
|  11  | Detection Height Bottom             | 検出する点群の高さの下限を指定する。                         |   m   |        |
|  12  | Deceleration for Obstacle           | 物体を検出して停止・減速する際の減速度を指定する             | m/s^2 |        |
|  13  | Deceleration for Stopline           | 停止線を検出して停止・減速する際の減速度を指定する           | m/s^2 |        |
|  14  | Velocity Change Limit               | 現在速度と目標速度の差がこの値より大きくならないようにする。急ブレーキを回避するためのパラメタ | km/h  |        |
|  15  | Tempral Waypoint Size               | 次のノードに渡すためにPublishするwaypointの数を指定する      |  個   |        |
...
※設定詳細は、「Autoware 自動運転ソフトウェア入門」P177,179参照

|  #   | 項目名                     | 内容                                                         | 単位 | 備考 |
| :--: | -------------------------- | ------------------------------------------------------------ | :--: | ---- |
|  1   | Waypoint                   | 目標速度をwaypointファイルに定義されている速度から取得する   |  -   |      |
|  2   | Dialog                     | Velocityパラメタから取得する                                 |  -   |      |
|  3   | Velocity                   | 目標速度（固定値）                                           | km/h |      |
|  4   | Lookahead Distance         | 目標点までの距離（固定値）                                   |  m   |      |
|  5   | lookahead_ratio            | 目標点を決めるパラメタ。その速度で何秒先に到達する位置を目標点とするかを表す |      |      |
|  6   | minimum_lookahead_distance | 目標点までの距離の最低値                                     |  m   |      |
...
Runtime Managerの［Computing］タブを選択し、［/vehicle_cmd］トピックがPublishされていることを確認する。



## SMAGVの自動運転切り替え

ジョイスティックの［Y］ボタン押下で自動運転に切り替え、AGVが自動走行することを確認する。
...
|         ボタン         | 内容                                               | 備考 |
| :--------------------: | -------------------------------------------------- | ---- |
|           Ｘ           | 手動走行（Bluetoothゲームパッドによる制御）        |      |
|           Ｙ           | 自動走行                                           |      |
|           Ｂ           | アカデミックパック本体のジョイスティックによる制御 |      |
| Ａ＋左スティック押込み | 緊急停止（アカデミック電源Off）                    | 仮   |
